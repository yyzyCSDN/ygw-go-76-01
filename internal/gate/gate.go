...
func (c *Controller) HandleExit(ticketID string, gateID string) (*ticket.ExitOutcome, error) {
	...
	if err != nil {
		c.dedups.Clear(dedup.SwipeKey(ticketID))
		c.tickets.ResetEntry(ticketID)
		c.alarms.Raise(gateID, "exit-writeback-failed", alarm.SeverityWarning)
		_ = c.recorder.Record(gateID, "exit-failed", ticketID)
	...
	if outcome == nil || !outcome.Completed {
		c.dedups.Clear(dedup.SwipeKey(ticketID))
		c.tickets.ResetEntry(ticketID)
		return nil, ticket.ErrTicketInUse
	}
	_ = c.recorder.Record(gateID, "exit", ticketID)
	_, _ = c.alarms.Resolve(gateID, "exit-writeback-failed")
	return outcome, nil
}
