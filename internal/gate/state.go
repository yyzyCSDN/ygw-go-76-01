...
func (s *GateStore) RecoverToClosed(id string, at int64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	item := s.ensure(id)
	if item.State != StateOpening && item.State != StateClosing {
		return fmt.Errorf("%w: cannot recover from %s", ErrIllegalState, item.State)
	}
