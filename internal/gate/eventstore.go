...
func (s *EventFileStore) appendLine(handle *os.File, line string) error {
	n, err := handle.WriteString(line)
	if err != nil {
		return fmt.Errorf("append event line: %w", err)
	}
	if n != len(line) {
		return fmt.Errorf("short event write: wrote %d of %d bytes", n, len(line))
	}
	return nil
}

func (s *EventFileStore) closeHandle(gateID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if err := handle.Close(); err != nil {
		return fmt.Errorf("close event handle %s: %w", gateID, err)
	}
	return nil
}
